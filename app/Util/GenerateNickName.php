...
class GenerateNickName
{
    ...
    public static function generate()
    {
        $rand = mt_rand(0, 3);
        if ($rand) {
            $surname       = self::getSurnameList();
            $surname_count = count($surname);
            $name          = self::getNameList();
            $name_count    = count($name);

            $ok_surname = $surname[mt_rand(0, $surname_count - 1)];
            $ok_name    = $name[mt_rand(0, $name_count - 1)].$name[mt_rand(0, $name_count - 1)];
        } else {
            $surname       = self::getFirst();
            $surname_count = count($surname);
            $name          = self::getlast();
            $name_count    = count($name);

            $ok_surname = $surname[mt_rand(0, $surname_count - 1)];
            $ok_name    = $name[mt_rand(0, $name_count - 1)];
        }

        return $ok_surname.$ok_name;
    }

    //获取姓氏
    private static function getSurnameList(){
        $surname = ['赵','钱','孙','李','周','吴','郑','王','冯','陈','褚','卫','蒋','沈','韩','杨','朱','秦','尤','许','何','吕','施','张','孔','曹','严','华','金','魏','陶','姜','戚','谢','邹',
    ...
    //获取名字
    private static function getNameList(){
        $name = ['伟','刚','勇','毅','俊','峰','强','军','平','保','东','文','辉','力','明','永','健','世','广','志','义','兴','良','海','山','仁','波','宁','贵','福','生','龙','元','全'
    ...
    {
        $first = ['狂奔的', '稳健的', '无敌的', '暴躁的', '幽默的', '老练的', '蛮力之', '咆哮之'];
        return $first;
    }
